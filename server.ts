...
async function authenticateToken(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  ...
  jwt.verify(token as string, getKey, { algorithms: ['RS256'] }, async (err, decoded) => {
    ...
    if (decodedPayload.role && decodedPayload.role.includes('{{')) {      
      ...
      req.user = {
        role: decodedPayload.role || 'basic',
...
function checkApiAccess(_apiPath: string, requiredRole: string) {
...
function createApiProxy(config: ProxyConfig): RequestHandler {
  ...
  const proxyOptions: any = {
    ...
    on: {
      proxyRes: (proxyRes: any) => setCorsHeaders(proxyRes)
    }
  };

  if (onProxyReq) {
    proxyOptions.on.proxyReq = onProxyReq;
  }
  
  return createProxyMiddleware(proxyOptions) as RequestHandler;
}
